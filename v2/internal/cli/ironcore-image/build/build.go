...
import (
	...
	_ "github.com/ironcore-dev/ironcore-image/v2/image/direct"
...
func Run(ctx context.Context, store *oci.Store, buildPath string, opts Options) error {
	...
	for _, cfgFile := range files {
		kind, err := image.DetectConfigKindFromFile(cfgFile)
		if err != nil {
			return err
		}

		buildPlatforms := opts.Platforms.CombinedFor(kind)
		...
		for _, platform := range buildPlatforms {
			...
			desc, _, err := image.BuildFile(ctx, pusher, buildCtx, cfgFile, image.BuildFileOptions{
				Platform: &p,
			})
			if err != nil {
	...
	if len(descs) > 1 {
		indexDesc, err := pack.Index(ctx, store, pack.IndexOptions{Manifests: descs})
		if err != nil {
